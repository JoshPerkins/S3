...
function multipartDelete(authInfo, request, log, callback) {
    ...
    async.waterfall([
        ...
        function getMPUMetadata(mpuBucket, mpuOverviewArray, destBucket, next) {
        ...
        function deleteData(mpuBucket, storedParts, destBucket, location,
        mpuOverviewKey, next) {
            ...
            if (locations.length === 0) {
                return next(null, mpuBucket, storedParts, destBucket, location,
                mpuOverviewKey);
            }
            // flatten the array
            ...
            }, () => next(null, mpuBucket, storedParts, destBucket, location,
                        mpuOverviewKey));
        },
        function deleteMetadata(mpuBucket, storedParts, destBucket, location,
        mpuOverviewKey, next) {
            ...
            services.batchDeleteObjectMetadata(mpuBucket.getName(),
                keysToDelete, log, err => {
                    ...
                    return next(null, destBucket, location);
                });
        },
        function ifMultipleBackend(destBucket, location, next) {
            if (config.backends.data === 'multiple') {
                return multipleBackendGateway.abortMPU(objectKey,
                uploadId, location, log, err => {
                    if (err) {
                        return next(err, destBucket);
                    }
